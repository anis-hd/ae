...
def build_compression_encoder(input_shape, latent_dim, num_filters=96):
    """Builds a lighter encoder potentially better suited for compression."""
    ...
    activation_fn = tf.keras.layers.ReLU() # Use ReLU

    # Layer 1: Downsample H/2, W/2
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=5, strides=2, padding='same', name='enc_conv1')(x)
    x = activation_fn(x)

    # Layer 2: Downsample H/4, W/4
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=5, strides=2, padding='same', name='enc_conv2')(x)
    x = activation_fn(x)

    # Layer 3: Downsample H/8, W/8
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=5, strides=2, padding='same', name='enc_conv3')(x)
    x = activation_fn(x)

    # Layer 4: Final projection to latent_dim
    latent = tf.keras.layers.Conv2D(latent_dim, kernel_size=3, strides=1, padding='same', name='encoder_output')(x)
    # Output dtype will be compute_dtype (e.g., float16)

    print(f"Compression Encoder Output shape (symbolic): {latent.shape}")
    return tf.keras.Model(inputs, latent, name='flow_compression_encoder')



def build_compression_decoder(latent_shape, latent_dim, num_filters=96):
    """Builds a lighter decoder using UpSampling2D, potentially better suited for compression."""
    latent_inputs = tf.keras.Input(shape=latent_shape, dtype=tf.keras.mixed_precision.global_policy().compute_dtype)
    x = latent_inputs
    activation_fn = tf.keras.layers.ReLU() # Use ReLU

    # Layer 1: Upsample H/4, W/4
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=3, strides=1, padding='same', name='dec_conv1')(x)
    x = activation_fn(x)
    x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest', name='dec_upsample1')(x) # Match encoder stride

    # Layer 2: Upsample H/2, W/2
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=5, strides=1, padding='same', name='dec_conv2')(x)
    x = activation_fn(x)
    x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest', name='dec_upsample2')(x) # Match encoder stride

    # Layer 3: Upsample H, W
    x = tf.keras.layers.Conv2D(num_filters, kernel_size=5, strides=1, padding='same', name='dec_conv3')(x)
    x = activation_fn(x)
    x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest', name='dec_upsample3')(x) # Match encoder stride

    # Final layer to reconstruct the 2-channel flow map (float32)
    outputs = tf.keras.layers.Conv2D(2, kernel_size=5, padding='same', activation='linear', dtype='float32', name='decoder_output')(x) # H, W, 2

    print(f"Compression Decoder Output shape (symbolic): {outputs.shape}")
    return tf.keras.Model(latent_inputs, outputs, name='flow_compression_decoder')
class FlowVQAutoencoder(tf.keras.Model):
    def __init__(self, input_shape, latent_dim, num_embeddings, commitment_cost, num_filters=96, **kwargs): # Added num_filters
        super().__init__(name="FlowVQAutoencoder_Light", **kwargs) # Changed name for clarity
    ...
    # --- Properties and other methods (call, train_step, test_step, etc.) remain UNCHANGED ---
    @property
